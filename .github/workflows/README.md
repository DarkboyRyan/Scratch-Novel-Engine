...
Pull Request、`main` push 或人工调度首先进入 `player-ci.yml`，在 macOS arm64、Windows x64 和 Linux x64 上执行 Runtime、Player、Editor 与 C++ 检查，并验证通用及嵌入式 Player 制品。`editor-ci.yml` 只接受人工调度，在 macOS arm64 与 Windows x64 原生 Runner 上重新执行测试、构建 C++ 后端和同平台 Player 模板，再完成 Editor 打包、内部签名分类、ZIP 回读与 SHA-256 校验。它不上传 Runner 临时文件，也不创建 Release。
...
| [`editor-ci.yml`](./editor-ci.yml) | GitHub Actions、Electron Forge、CMake | 人工执行的 Editor 双平台内部打包验证。 | macOS arm64 / Windows x64、无 Secrets、ZIP 原生回读；不上传制品。 |
...
- 当前 Editor 工作流不得上传仓库制品或创建 GitHub Release；正式 Editor 发布自动化尚未在本目录启用。
- 正式发布缺少证书、公证凭据或 GPG 材料时必须失败，不能生成未签名替代品。
- 临时证书、钥匙串和下载的 Bundle 只能放在 Runner 临时目录，不应写回工作区或上传为普通日志。

## 修改与验证

先按工作流顺序运行 Runtime、Player、Editor 和 C++ 的本地检查；涉及 Editor 打包时还应运行 `pnpm --dir apps/editor test:release-tools`。Pull Request 中至少观察三平台内部矩阵；Editor 原生包边界可人工运行 `editor-ci.yml`，但其成功只证明内部 ZIP 通过校验，不会留下可下载制品。正式 Player 发布流程的改动应通过受保护环境中的候选演练验证，而不是直接用正式标签试错。
