...
Pull Request、`main` push 或人工调度首先进入 `player-ci.yml`，在 macOS arm64、Windows x64 和 Linux x64 上执行 Runtime、Player、Editor 与 C++ 检查，并验证通用及嵌入式 Player 制品。`editor-ci.yml` 接受人工调度，并为初次交付精确监听 `feature/editor-release` 分支 push；它在 macOS arm64 与 Windows x64 原生 Runner 上重新执行测试、构建 C++ 后端和同平台 Player 模板，再完成 Editor 打包、内部签名分类、ZIP 回读与 SHA-256 校验，最后把已验证候选短期上传为 Actions artifact。它不会创建 Release。
...
| [`editor-ci.yml`](./editor-ci.yml) | GitHub Actions、Electron Forge、CMake | 受控执行的 Editor 双平台内部打包验证。 | macOS arm64 / Windows x64、无 Secrets、ZIP 原生回读、7 天 internal artifact。 |
...
- Editor 工作流只上传经过完整回读的 internal 候选目录，保留 7 天；它不得创建 GitHub Release，正式 Editor 发布自动化尚未在本目录启用。
- 正式发布缺少证书、公证凭据或 GPG 材料时必须失败，不能生成未签名替代品。
- 临时证书、钥匙串和下载的 Bundle 只能放在 Runner 临时目录，不应写回工作区或上传为普通日志。

## 修改与验证

先按工作流顺序运行 Runtime、Player、Editor 和 C++ 的本地检查；涉及 Editor 打包时还应运行 `pnpm --dir apps/editor test:release-tools`。Pull Request 中至少观察三平台内部矩阵；Editor 原生包边界可运行 `editor-ci.yml`，其成功会留下保留 7 天的可下载 internal ZIP 与校验元数据，但不代表正式签名发行。正式 Player 发布流程的改动应通过受保护环境中的候选演练验证，而不是直接用正式标签试错。
