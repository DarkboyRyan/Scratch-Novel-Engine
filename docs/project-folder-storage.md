<!-- 文件职责：说明项目文件夹与媒体存储；关键内容：目录契约、原子保存、安全导入和 capability URL。 -->
...
Scratch Novel Engine 的项目不再以用户手动选择的单个 JSON 文件为单位，而是以一个
...
`displayName` 保存，因此两个同名源文件不会相互覆盖。新导入的同类资源若显示名
重复，Backend 会保留首个名称，并为后续资源依次生成 `名称 (2)`、`名称 (3)`；
图片、音频和视频各自使用独立的显示名命名空间。
