...
> v19 增加人物 `mode` 并区分待选图占位与明确清除；当前 Writer 为 v19、Reader 支持
> v1–v19。显示 CG 的完整历史里程碑是 Author v17 / Runtime v8 / Snapshot v3；
...
> 七类人物动画、右侧 value socket、原子移动、Runtime v9、Snapshot v4 与暂停/
...
Author 占位，预览会跳过，所有导出格式都会以稳定错误拒绝；它不会降级成 Runtime v9 的
清除指令。完整 effect union 示例见[人物立绘特效](./character-portrait-effects.md)。

兼容策略：

- 人物节点在 v5 首次加入。
- 当前 Reader 严格接受 v1–v19。
- v1/v2 只有对白节点。
- v3 支持必须绑定图片的背景节点。
- v4 支持 `assetId: null` 的背景节点。
- v5 支持人物节点。
- v6 支持场景跳转；v7 增加语音/BGM；v8 增加 VideoNode；v9 增加 ChoiceNode；
  v10 增加项目级 `startScreen` 媒体，v11 增加独立标题，v12 增加手动延伸，v13 为人物节点增加可空 `position: {x,y}`，v14 增加扁平 CG 画廊，v15 把画廊升级为固定九槽页面，v16 增加变量和配对逻辑，v17 增加显示 CG 控制块，v18 增加人物 sidecar effect，v19 增加人物 `mode`；当前 Writer 始终写 v19。
...
12. v1–v18 项目仍能打开并在保存时升级到当前 v19；v1–v12 的旧人物节点自动补 `position: null`，v13 及之后的坐标保持不变，v1–v17 人物统一补 `effect: null`，并按旧 assetId 推导 mode。
