...
| 文件格式 | nlohmann/json、版本化 Reader/Writer | v9 引入嵌套 `options`；当前 v22 Writer 延续严格字段 |
...
## 4. Choice 在 v9 引入，当前项目文件为 v22

当前 Writer 固定写 `fileVersion: 22`，Reader 接受 v1–v22。`type: "choice"` 是
...
| C++ 序列化 | v9 Choice 迁移、当前 v22 round-trip、空/多选项、严格字段、旧版本拒绝 Choice、损坏引用拒绝 |
...
> 返回完整快照。Choice 当时随项目文件 v9 引入；当前 Writer 已升级到 v22，并继续
