...
- Player 兼容 runtime v1–v12；Runtime v11 是场景初始背景、时间线背景与人物立绘
  ...
  可编辑标题上方文字、独立标题、自定义背景、循环标题音乐和固定的
...
- `BackgroundNode`：设置图片或显式切换为无背景；图片使用 10–300 的整数缩放，空背景
  必须为 100；
- `CharacterNode`：`mode:'show'|'clear'` 明确区分显示/待选图占位与清除某人物层，
  可用百分比坐标覆盖左/中/右预设，并可在执行时发出一次震动、跳跃、呼吸、闪烁、
  淡入、淡出或滑入 effect；立绘使用 10–300 的整数缩放，clear 必须为 100；
...
作者项目或只读 runtime bundle。Renderer 只传小型 `GameRuntimeSnapshot v5`，Main 依据
当前 bundle identity 恢复并校验，固定槽通过临时文件、fsync、备份和 rename 发布。
快照 v5 在 v4 的全局特效序号与人物最终 `opacity`/分层序号之外保存背景和各人物层缩放，
恢复时清除瞬时 effect，避免动画重播；旧 v1–v4 快照仍按各自能力受限恢复并补 100% 缩放，
其中 snapshot v3 是显示 CG 状态、snapshot v4 是人物特效状态的历史里程碑。
...
音量均为 1、窗口模式为 windowed、尺寸为 medium。无持久语言或旧 v1 设置时，
桌面和 Web Player 使用当前包的 `defaultLanguage`；已持久 v2 玩家语言优先。共享 typed catalog 与 React Context
