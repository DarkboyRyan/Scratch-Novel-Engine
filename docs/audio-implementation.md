<!-- 文件职责：记录语音与背景音乐链路；关键内容：导入、节点模型、预览、Player 播放和安全边界。 -->
...
| 持久化 | nlohmann/json、音频引入于 fileVersion v7 | 严格保存 `voiceAssetId` 和 BGM 节点；当前 Writer 为 v19 |
...
音频在 v7 引入；当前 Writer 固定写 v19，Reader 读取 v1–v19。Reader 会把
