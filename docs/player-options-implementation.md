...
- 作者项目固定写 `fileVersion: 16`，Reader 支持 v1–v16；
- 导出内容为 runtime v7，Player 兼容 runtime v1–v7；
- 游戏进度使用独立的 `GameRuntimeSnapshot v2` 和 `saveVersion: 1`，并受限兼容无逻辑旧 v1；
...
- 不会调整 Editor BrowserWindow，也不会把设置写入 author v16 或 runtime v7。
