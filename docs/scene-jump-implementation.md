...
| 文件格式 | nlohmann/json、版本化严格 Reader/Writer | 跳转在 v6 引入；当前 v20 仍严格保存 `targetSceneId` |
...
| 持久化 | 跳转在 v6 引入；当前 Writer v20、Reader v1–v20 | `engine/src/backend/serialization.cpp` |
...
- 错误码稳定；
- v6 来源文件打开后，保存为 v20 并重新打开仍保留目标 ID；
- v1–v19 均能读取并升级为当前 v20 写出格式；
...
这一阶段 Reader 接受 v1–v6、Writer 写 v6；当前 Reader 接受 v1–v20、Writer
写 v20。Project/Scene `schemaVersion` 仍保持 1。
