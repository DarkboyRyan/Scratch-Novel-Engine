...
| 文件格式 | nlohmann/json、版本化严格 Reader/Writer | 跳转在 v6 引入；当前 v13 仍严格保存 `targetSceneId` |
...
| 持久化 | 跳转在 v6 引入；当前 Writer v13、Reader v1–v13 | `engine/src/backend/serialization.cpp` |
...
- 错误码稳定；
- v6 来源文件打开后，保存为 v13 并重新打开仍保留目标 ID；
- v1–v12 均能读取并升级为当前 v13 写出格式；
...
这一阶段 Reader 接受 v1–v6、Writer 写 v6；当前 Reader 接受 v1–v13、Writer
写 v13。Project/Scene `schemaVersion` 仍保持 1。
