<!-- 文件职责：记录 Web Player ZIP 导出；关键内容：浏览器模板、Gateway、IndexedDB、ZIP 安全和部署。 -->

# Web Player ZIP 导出实现

> 实现目标：Editor 的导出面板新增“Web 游戏 ZIP（HTML5）”，把当前已保存的作者项目
> 编译为可部署到静态网站的浏览器版游戏。它沿用 Runtime v9、共享 React Player UI 和
...
```mermaid
flowchart LR
  AUTHOR["作者项目 v19<br/>project.vn.json + assets"]
  EDITOR["Electron Editor Main<br/>冻结 revision 与严格编译"]
  RUNTIME["Runtime Bundle v9<br/>game.json + manifest + assets"]
...
- Runtime Compiler 继续把 author v19 编译为 runtime v9，不为 Web 复制另一套剧情语义；
...
指定的模板路径。模板可以读取 runtime v1–v9；当前新导出固定生成 runtime v9，且
`web-export.json` 声明 `playerCompatibility: ">=9 <10"`。
...
`prefers-reduced-motion: reduce` 会禁用位移动画，但 Runtime 仍提交相同的最终 opacity，
因此无障碍设置不会改变剧情或存档结果。
...
| Runtime 编译 | TypeScript、现有 Runtime Bundle Exporter | author v19 → runtime v9，逻辑结构、资源闭包、人物 effect/mode、hash、媒体魔数和源稳定性验证 |
...
| 剧情状态机 | `@vnengine/runtime` | 复用 runtime v9 逻辑/立绘特效语义与 `GameRuntimeSnapshot v4` |
...
- 正常 HTTP 响应可加载 runtime v9、逻辑节点、人物 effect 和全部 Asset ID；
...
- [人物立绘特效实现](./character-portrait-effects.md)：严格联合、运行时动画、暂停、
  `prefers-reduced-motion` 与存档恢复语义。
