<!-- 文件职责：提供工程技术栈讲解指南；关键内容：架构话术、调用链、技术取舍和常见问答。 -->

# Scratch Novel Engine 技术栈与面试讲解指南
...
- Editor 默认进入排在“场景 1”之前的软件托管主界面，可用表单或固定 Blockly 结构配置
  标题上方文字、独立游戏标题、背景图片和背景音乐，并预览完整标题页流程；
- 独立的软件托管 CG 画廊编辑场景：表单手动新增/删除页面并编辑固定九槽，Blockly
  由作者从工具箱拖入每页大模块；空项选择“无”，资源面板点击不直接加入；
- 表单编辑与 Blockly 图形化编辑；作者可用编号“延伸”积木主动拆分长剧情；
- 分类剧情 Toolbox、变量 Set/Change、可嵌套 If/Else、固定次数 Repeat、只容纳对白的
  “显示 CG”C 形积木，以及人物右侧 typed value socket 的七类立绘特效；表单以只读树和
  静态特效摘要展示结构，正式预览执行真实变量、控制流、CG 计时和动画；
- 项目文件夹保存、打开和未保存状态；
- PNG/JPEG/WebP 图片、MP4/WebM 视频与 MP3/WAV/Ogg 音频安全导入；
- 对白语音和时间线 BGM，正式预览使用独立双音轨控制器；
- 正式顺序预览、阻塞式视频/选项、鼠标/键盘推进和跳转循环检测；
- 共享 Runtime/Player UI、v22→runtime v13 `.vngame` 目录导出和通用 Player；
- Player 兼容 runtime v1–v13；Runtime v11 首次保存场景初始背景、时间线背景与人物立绘
  的 10%–300% 整数缩放，当前 v13 继续支持；标题页背景和 CG 不参与该剧情图片缩放契约。标题页渲染可编辑标题上方文字、
...
> `std::variant` 表达普通剧情、变量、paired logic/CG markers 与作者专用
...
> 不写进 `project.scenes`；固定根积木编辑标题上方文字与独立游戏标题，并包裹背景图片和背景音乐积木，拖入资源后通过
...
> “显示 CG”也用 root/end marker 投影为 Dialogue-only 的 C 形块，图片真正 load/decode 后
> 才开始可暂停的对白前计时，Desktop、Web 和 Editor 正式预览复用同一 Runtime。
> 人物立绘特效则是 CharacterNode 的严格 sidecar union，而不是新增时间线节点；七个 value
> block 接到人物积木右侧，跨人物拖动由单条 `characterEffect.move` 原子清源写目标。共享
> VisualStage 等图片 load/decode 后播放动画，并统一处理暂停、页面隐藏和 reduced motion。
>
...
>
> 游戏导出复用既有 C++ 保存链冻结 v22 和 revision；Editor Main 再严格编译
> runtime v13，只复制剧情、主界面与 CG 非空槽引用媒体，并通过同盘 staging、SHA-256 和原子
> rename 发布，同时把 Main 权威 Editor 语言写入 `game.defaultLanguage`。Player 兼容
> runtime v1–v13，旧 v1–v11 的包默认语言迁移为中文，旧 v1–v12 的页面样式迁移为安全默认值；标题音乐由标题页独立控制，进入剧情
...
> author v22、runtime v13 或 `GameRuntimeSnapshot v5`。
...
| 图形化编辑 | Blockly 13.1 | 分类 Toolbox、变量与 C 形逻辑、编号延伸分页、人物右侧特效 value socket、主界面固定积木树，以及固定九槽 CG 页模块 |
| 样式与画面 | HTML、CSS | 背景、立绘、七类 CSS 动画、暂停/reduced-motion、对白框及编辑器布局；当前没有使用 Canvas/Pixi |
...
| Runtime 导出 | TypeScript strict parser、Node streams、SHA-256 | 已保存 v22→runtime v13、页面样式、eyebrow、剧情图片缩放、包默认语言、严格逻辑/effect/mode 结构、资源闭包与 staging 原子发布 |
...
`Project` 还持有项目级 `StartScreen`，其中 eyebrow 是可为空的标题上方文字，标题是独立于项目名的非空文本，背景图片和
音乐则是可空 Asset ID。它不是剧情 `SceneNode`：标题页先于剧情运行，但不会参与
`Scene.nodes` 顺序或场景跳转。eyebrow 默认是 `A VN ENGINE STORY`，空值隐藏；C++ 会
trim 首尾 ASCII 空白并拒绝 NUL 或超过 256 个 UTF-8 字节的值，同时在一次候选提交中
校验标题非空、背景只能引用 image、音乐只能引用 audio；非法输入不产生部分修改，相同配置是 no-op。
...
- 显示 CG 使用另一种 paired C 形范围，白色图片下拉与秒数字段由专用命令更新，body
  只接受 Dialogue；
- 七类人物特效是 value block，只能接到非清除人物积木右侧的 typed socket；字段修改、
  拔出/删除和跨人物拖动分别映射到窄 update 或原子 move 命令；
...
表单会隐藏它，Compiler 会在生成 runtime v13 前剥离，因此它没有游戏运行行为。
...
图片分类中的“显示 CG”同样以 paired range 保存，但它的 body 只接受对白。Player 等图片
实际就绪后再计算 lead-in，暂停、存读档、选项和隐藏页面都保留剩余时长。完整实现见
[显示 CG Blockly](./cg-display-blockly-implementation.md)。

人物积木的特效 socket 只接受 `shake / jump / breathe / flash / fadeIn / fadeOut / slideIn`
七种判别联合。表单只读显示摘要，静态时间线预览只应用最终 opacity，不播放动画。完整实现见
[人物立绘特效](./character-portrait-effects.md)。

主界面提供表单和独立 Blockly 工作区；Blockly 固定投影为“标题上方文字 + 主界面游戏名根积木 → 背景图片
→ 背景音乐”。三个积木均不可移动、删除或改写结构，也没有 Toolbox；根积木包含白色
标题与 eyebrow 输入，两个资源子积木使用白色下拉框，第一项固定为“无”，并保留对应类型资源拖放。
表单模式在右侧内容区提供同样的 eyebrow/标题输入、两个白色选择框和标题页设计预览。两种视图都只操作 C++ 快照中的
...
当前 Writer 写 `fileVersion: 22`，Reader 支持 v1–v22。v9 曾新增 ChoiceNode 的
...
读取 v14 时按顺序每九张分块并补 `null`，读取 v1–v13 时生成一张全空页；v16
新增变量、严格条件 AST 和 If/Else/Repeat paired markers；v17 新增显示 CG 的
...
人物节点新增 10–300 整数缩放；v22 为主界面和 CG 画廊新增严格类型化页面样式。旧 v1–v17 人物迁移为
`effect:null`，旧 v1–v18 人物按 `assetId` 迁移 mode，v1–v19 eyebrow 补
`A VN ENGINE STORY`，旧 v1–v20 缩放补 100%，旧 v1–v21 页面样式补安全默认值；下一次保存统一写 v22，未来版本仍被拒绝。`show` 可在编辑阶段以
...
引入；Runtime v11 首次加入图片缩放，当前为 runtime v13 / snapshot v5，其中 Runtime
...
- `StartScreen` 是项目级标题页配置，不是剧情节点；标题上方文字和标题可独立修改，
  eyebrow 为空会隐藏且受 256 UTF-8 字节/NUL/trim 边界约束，标题不能为空，背景和音乐可分别设为“无”。
- `CgGallery` 也是项目级配置，不是剧情节点；`pages` 至少一页，每页九槽，非空
  `imageAssetIds` 跨页唯一且必须引用现有 image Asset，空槽的位置同样具有语义。
- 人物节点修改 1–10 中的一层，保存图片、左中右位置、layer 和严格可空 sidecar effect；
  高层渲染在前，清除立绘时 effect 必须为空。
...
人物图片完成 load/decode 后，共享 `VisualStage` 才启动 shake、jump、breathe、flash、
fadeIn、fadeOut 或 slideIn。Runtime 使用全局单调 `characterEffectSequence` 保证循环再次
执行同一节点仍产生新的 React key；清除立绘和场景跳转不归零。暂停菜单、阻塞弹层、媒体
阻塞和页面隐藏只暂停 CSS animation 进度，`prefers-reduced-motion` 禁用动画但不改变最终
opacity。表单时间线只展示静态最终状态。

剧情背景和人物立绘缩放由 Runtime v11 首次引入、当前 Runtime v13 延续的状态驱动：
...
独立 Player 在进入剧情状态机前先显示 runtime v13 的 `game.startScreen`。标题上方文字和独立标题直接来自
...
补空媒体或从 `game.title` 补标题；runtime v1–v9 eyebrow 补默认文案。runtime v6–v13 严格读取固定页面；runtime v5 的扁平
列表按序分块补空，v1–v4 规范化为一张全空页。Runtime v12 引入的 `game.defaultLanguage`
由 Editor Main 写入导出时权威语言，旧 runtime v1–v11 迁移为 `zh-CN`；Runtime v13
要求主界面和 CG 画廊的严格样式 DTO，旧 runtime v1–v12 补安全默认值。
...
- macOS Editor package/make 会先生成 exact Player 模板并复制到
  `Resources/player-templates`；模板声明 `runtimeCompatibility: ">=1 <14"`，CI 会
...
Editor 新建/打开/初始化项目
  ...
  → 用户选择表单或固定“标题上方文字 + 主界面游戏名 → 背景图片 → 背景音乐”Blockly 树
  → 填写标题上方文字与独立标题，通过白色下拉框选择素材、“无”，或将图片/音频拖入对应积木
  → useEngineProject.updateStartScreen
  → Preload / Main 校验 exact params
  → C++ 校验 eyebrow 的 trim/NUL/256 UTF-8 字节边界、标题非空、Asset 存在且类型匹配并原子提交
...
Editor 点击“导出”
  ...
  → Main 确认 clean/saved revision，稳定读取磁盘 v22
  → TypeScript 严格编译 runtime v13，并注入 Main 权威 Editor 语言，只复制剧情、主界面与 CG 非空槽引用媒体
...
runtime v13 manifest 使用 `playerCompatibility: ">=13 <14"`；Player Reader 同时接受
runtime v1–v13，而当前 Player 模板用 `runtimeCompatibility: ">=1 <14"` 明确覆盖十三代
输入。运行 v13 时标题页使用可编辑标题上方文字、独立标题、页面样式与固定九槽 CG 画廊，并固定显示
...
没有 `window.vnPlayer` 设置调用。选项设置与作者项目 v22、runtime v13 和
`GameRuntimeSnapshot v5` 相互独立。

### 5.8 编辑、移动并播放人物立绘特效

```text
作者把特效 value block 接到人物积木右侧
  → Blockly 校验 typed socket、人物不是清除节点和字段值域
  → 新增/修改/拔出调用 characterEffect.update
  ...
  → Author v22 保存 strict union/scale/style → Compiler 输出 Runtime v13
...
- 项目 Writer 为 v22、Reader 支持 v1–v22；v9 保存 ChoiceNode/ChoiceOption，v10
  ...
  `show/clear` 意图，v20 新增可编辑标题上方文字，v21 新增剧情背景与人物立绘缩放，v22
  新增主界面与 CG 画廊严格页面样式；
- Editor 已完成 v22→runtime v13 内容包导出；Player 兼容 runtime v1–v13，packaged macOS
  Editor 还能通过 strict
