...
Current version boundary: the C++ Author Writer is v19 and its Reader accepts
v1–v19. Editor exports Runtime v9 with manifest compatibility `>=9 <10`;
Desktop/Web templates accept `>=1 <10`, and player progress uses Snapshot v4.
Author v17 / Runtime v8 / Snapshot v3 remains the historical “display CG”
milestone rather than the current contract.

## 模块导航

| 目录 / 文件 | 框架 / 技术 | 主要作用 | 关键实现 |
| --- | --- | --- | --- |
| [`include/`](./include/README.md) | C++20 公共 API | 暴露稳定领域模型和项目命令。 | `model.hpp`、`project.hpp`。 |
| [`src/core/`](./src/core/README.md) | C++20 | 无 JSON 依赖的权威业务核心。 | 节点原子变更、控制结构、聚合校验。 |
| [`src/backend/`](./src/backend/README.md) | C++20、nlohmann/json、JSONL | Electron Main 与 Core 之间的进程边界。 | 协议、迁移、媒体导入与原子文件发布。 |
| [`tests/`](./tests/README.md) | CTest | Core 与 Backend 的安全和兼容回归。 | 4 个独立测试目标。 |
| [`CMakeLists.txt`](./CMakeLists.txt) | CMake | 配置构建、依赖和测试目标。 | C++20、Backend executable、CTest。 |
...
`character.add`, `bgm.add`, `video.add`, `choice.add`, `storyExtension.add`,
`cgDisplay.add`, `variableSet.add`, `variableChange.add`, `logicIf.add`, and
`logicRepeat.add` return
...
- `background.add`, `background.update`, `background.delete`,
  `background.reorder`
- `character.add`, `character.update`, `characterEffect.update`,
...
- `storyExtension.add`
- `cgDisplay.add`, `cgDisplay.update`, `cgDisplay.delete`, `cgDisplay.reorder`
- `variableSet.add`, `variableSet.update`, `variableChange.add`,
  `variableChange.update`
- `logicIf.add`, `logicIf.update`, `logicRepeat.add`, `logicRepeat.update`,
  `logicControl.delete`, `logicControl.reorder`
- `sceneJump.add`, `sceneJump.update`
...
reads project file versions 1 through 19. Parsing and aggregate validation
...
Dialogue nodes; they migrate to the unified in-memory timeline. Later readers
migrate fixed-page CG galleries, paired logic controls, CG display ranges, and
nullable portrait effects without weakening the current aggregate validator.
Character nodes from versions 1 through 17 migrate to a null effect; an
`effect` field forged into one of those older versions is rejected, as is a
version 18 character node that omits the required nullable field. Author v19
adds `mode: "show" | "clear"`: older nodes migrate from `assetId` (`null`
becomes `clear`, an image becomes `show`), while `show` plus a null Asset is a
persisted block-editor placeholder and `clear` is the explicit layer-clear
command. `project.save` always writes version 19:
...
File version 18 adds nullable portrait effects. File version 19 distinguishes
an editable `show` placeholder from an explicit `clear` command with the
required character `mode` field. When older files are read, a null `assetId`
migrates to `clear` and a non-null image migrates to `show`.
...
`character.add` creates a center-positioned node on layer 1 and may assign an
initial image atomically. Its optional `mode` defaults to `show`, so omitting
`assetId` (or passing `null`) creates a persistent editable placeholder;
the Editor preview treats that unresolved show as a no-op and the exporter
rejects it until an image is selected. `mode: "clear"` creates the explicit
clear-layer command.
`character.update` atomically sets its nullable image, slot, layer, optional
exact position, and optional mode. Omitting mode preserves the existing
intent; explicit clear requires null Asset, position, and effect. Removing an
image from a show node also clears its effect. `characterEffect.update` attaches or clears a
strictly typed effect, and `characterEffect.move` atomically clears the source
and writes the same complete effect to a non-empty target portrait. A later
show node with an image replaces the portrait on the same layer, while an
explicit clear node clears that layer. For Blockly mixed selections, the `timeline.*` commands accept dialogue,
...
Portrait effects use an exact tagged union. `shake`, `jump`, `breathe`, and
`flash` require `durationMs` plus `intensity`; `fadeIn` and `fadeOut` require
only `durationMs`; `slideIn` additionally requires `intensity` and
`direction`. Duration is an integer from 100 through 10000 milliseconds,
intensity is `subtle|normal|strong`, and direction is
`left|right|up|down`. A cleared portrait must have a null effect.

```json
{"method":"characterEffect.update","params":{"sceneId":"scene-id","nodeId":"character-id","effect":{"type":"shake","durationMs":500,"intensity":"normal"}}}
{"method":"characterEffect.move","params":{"sceneId":"scene-id","fromNodeId":"character-a","toNodeId":"character-b","effect":{"type":"slideIn","durationMs":650,"intensity":"strong","direction":"left"}}}
```

Move verifies that the source still owns the exact effect supplied by the
caller and that the target has an image before changing either node. Missing
or mismatched source state, a cleared target, same-node moves, and invalid
unions leave both the aggregate and revision unchanged. See
[人物立绘特效实现](../docs/character-portrait-effects.md) for the complete
Editor, Runtime, Player, pause, reduced-motion, and Snapshot v4 chain.
...
revision. The next ordinary `project.save` persists the selection in the v19
