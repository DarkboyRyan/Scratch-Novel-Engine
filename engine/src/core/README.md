...
显式 `clear` 和七类 sidecar 特效使用严格判别约束。最终的 `validate_project_aggregate`
会再次检查场景、资源、CG 九槽、选择目标及所有嵌套结构。
...
| [`project_internal.hpp`](./project_internal.hpp) | 共享仅供 Core 使用的轻量辅助。 | 文本、位置、层级和特效参数检查 |
