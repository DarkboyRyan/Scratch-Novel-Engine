...
请求，严格解析字段并调用 Core；同时负责 Author v1–v21 迁移、v21 写出、安全媒体识别、
无覆盖资源导入和耐久原子保存。
...
校验后才替换会话；Writer 始终写 Author v21。v21 精确保存场景初始背景、时间线背景和
...
| [`serialization.hpp`](./serialization.hpp) / [`serialization.cpp`](./serialization.cpp) | 严格读取、迁移和写出 Author 文件。 | v1–v21 Reader、v21 Writer、标题上方文字、图片缩放与节点精确字段 |
