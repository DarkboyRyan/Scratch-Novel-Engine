...
| [`runtimeBundlePlayerCompatibility.test.ts`](./runtimeBundlePlayerCompatibility.test.ts) | Vitest、Editor Exporter、Player Loader | 验证 Editor 导出到 Player 读取的兼容性。 | 编译 Author v22、导出 Runtime v13，以页面样式、英文默认语言和非默认比例覆盖端到端契约，再用 `loadRuntimeBundle` 回读并调用 `startGame` 验证。 |
