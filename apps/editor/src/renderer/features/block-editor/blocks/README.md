...
2. 项目投影调用 `set*Block*` 辅助函数把作者节点写入稳定字段，资源名字段只截断显示、不改变真实值；背景与立绘缩放统一限制为 10%–300% 的整数。
...
| [backgroundBlock.ts](./backgroundBlock.ts) | TypeScript + Blockly | 注册背景积木并读写资源、缩放和本地化标签，兼容 HMR 旧实例 | `BACKGROUND_BLOCK_TYPE`、`BACKGROUND_BLOCK_FIELDS`、`applyBackgroundBlockLocalization`、`setBackgroundBlockAsset`、`setBackgroundBlockScalePercent`、`registerBackgroundBlock` |
...
| [characterBlock.ts](./characterBlock.ts) | TypeScript + Blockly | 注册人物立绘和清除立绘积木并读写位置、图片与缩放，兼容 HMR 旧实例 | `CHARACTER_BLOCK_TYPE`、`CLEAR_CHARACTER_BLOCK_TYPE`、`CHARACTER_BLOCK_FIELDS`、`applyCharacterBlockLocalization`、`setCharacterBlockScalePercent`、`getCharacterBlockScalePercent` 等 |
...
| [logicControlBlock.ts](./logicControlBlock.ts) | TypeScript + Blockly | 注册条件和循环控制积木；If 变量操作数使用项目级可搜索选择器，常量操作数使用文本输入 | `LOGIC_IF_BLOCK_TYPE`、`LOGIC_REPEAT_BLOCK_TYPE`、`LOGIC_CONTROL_INPUTS`、`LOGIC_CONTROL_FIELDS`、`LogicControlMarkers`、`readLogicIfBlock` 等 |
| [searchableVariableDropdown.ts](./searchableVariableDropdown.ts) | TypeScript + Blockly + DOM | 提供按变量名开头、忽略英文大小写筛选的变量选择弹层；查询文本不进入项目 | `SearchableVariableDropdown`、`filterVariableNamesByPrefix` |
...
| [variableBlock.ts](./variableBlock.ts) | TypeScript + Blockly | 注册变量新建/赋值积木，维护 If/Change 共用的项目级变量候选，并解析多类型逻辑值 | `VARIABLE_SET_BLOCK_TYPE`、`VARIABLE_CHANGE_BLOCK_TYPE`、`collectDeclaredVariableNames`、`setVariableNameFieldOptions`、`parseLogicValue` 等 |
