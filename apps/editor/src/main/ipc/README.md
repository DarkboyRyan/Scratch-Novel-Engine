...
| --- | --- | --- | --- |
| [`registerAssetIpc.ts`](./registerAssetIpc.ts) | Electron ipcMain、Dialog | 处理图片、音频和视频的导入、重命名、逻辑批量删除与预览 URL。 | `registerAssetIpc`；校验来源、独占文件操作、撤销被删除资源的预览 capability，并更新窗口状态。 |
| [`registerEditorSettingsIpc.ts`](./registerEditorSettingsIpc.ts) | Electron ipcMain | 处理 Editor 语言与颜色主题设置的读取、窄 patch 更新和跨窗口广播。 | `isEditorSettingsInvocation`、`broadcastEditorSettings`、`registerEditorSettingsIpc`。 |
...
| [`validateAssetInvocation.ts`](./validateAssetInvocation.ts) | TypeScript Guard | 校验资产导入、查询和管理参数。 | `isAssetInvocation` 仅允许白名单动作、精确字段、受限名称和无重复 ID 批次。 |
