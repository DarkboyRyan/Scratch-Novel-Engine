...
单游戏桌面构建通常先验证环境与 Runtime Bundle，再准备嵌入资源并调用 Electron Forge。产物随后经过平台结构检查、可选签名、定位与归档，最后由收集和发布集脚本核对跨平台文件及收据。macOS 与 Windows x64 的通用 Player 还可暂存为同平台 Editor 模板；Web 流程独立构建静态 Player，将允许的 payload 和哈希清单暂存成模板，再由 Editor 写入游戏内容。
