...
不能突破预算，人物特效必须满足严格字段组合，旧快照只能在仍可证明安全的状态下恢复。

## 文件索引

| 文件 | 主要覆盖 | 关键合同 |
| --- | --- | --- |
| [`gameRuntime.contract.test.ts`](./gameRuntime.contract.test.ts) | 对白、媒体、选择、逻辑、循环、CG 和人物状态。 | 推进停点、错误语义、控制流与事件序号 |
| [`gameRuntimeSnapshot.test.ts`](./gameRuntimeSnapshot.test.ts) | Snapshot v1–v4 校验、比较与恢复。 | 往返等价、旧版兼容、拒绝不一致状态 |
